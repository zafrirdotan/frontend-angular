...
type GetAddTextFunction = (availableItems?: any, unavailableItems?: any) => string
...
} =
{
    ...
    introductionMessage: {
        he: () => 'שלום, אני עוזר הקניות שלך. תוכל לכתוב לי את רשימת הקניות שלך בטקסט פשוט ואני אוסיף אותה לעגלה שלך',
        en: () => 'Hi, I am your shopping assistant. Please write your shopping list in plane text and I will add it to your cart.',
    },
    addingItemsToCart: {
        he: () => 'הפרטים נוספו לעגלה שלך',
        en: (availableItems, unavailableItems) => {
            if (availableItems?.length) {
                return `Sure! I have added the flowing items to your cart ${availableItems?.map((item: any) => `\n- ${item.quantity} ${item.unit} ${item.name}`)}
                ${unavailableItems?.length ? `\nbut I could not find the following items ${unavailableItems.map((item: any) => `\n- ${item.name}`)}` : ''}`
            } else if (unavailableItems?.length) {
                return `Sorry, I could not find the following items ${unavailableItems.map((item: any) => `\n- ${item.name}`)}`
            }
    ...
    ,
    addingX: {
        he: (action: any) => `לעגלת הקניות ${action?.list[0]?.name} ${action.quantity} נוספו עוד`, // problem 
        en: (availableItems: any) => `I have added the flowing items to your cart ${availableItems?.map((item: any) => `\n- ${item.quantity} ${item.unit} ${item.name}`)}`,
    },
    ...
    removingX: {
        he: (action: any) => `מעגלת הקניות ${action?.list[0]?.name} ${action.list[0]?.quantity} הוסרו`,
        en: (action: any) => `I have removed ${action.list[0]?.quantity} ${action?.list[0]?.name} from the cart`,
    },
